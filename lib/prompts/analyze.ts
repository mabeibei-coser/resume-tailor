...
{
  "suggestions": [  // 数组，长度 1-5 条 — 这是「简历问题项」，只列出简历里真正需要被优化的具体问题点；简历质量好就少报几条，别凑数
    {
...
建议应覆盖两类（如果只有 1 条建议可以只属于一类）：
- 类型 A：「简历自身写得不恰当」（措辞 / 结构 / 量化 / 动词等通用质量问题）
- 类型 B：「与目标 JD 不匹配」（JD 关键能力 / 关键词 / 经历类型在简历中缺失或不突出）
有几条真实问题就写几条，不要为了凑数把无关紧要的小问题也列上去。
