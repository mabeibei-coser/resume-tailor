...
**用户期望**：激进型，可基于简历已有内容合理联想，虚构及润色程度 < 30%；重点突出与目标岗位匹配的【能力表现 / 业绩表现 / 项目经历】。

- **可大幅重写措辞 / 可基于简历内容做合理联想推测数字**：从已有项目延伸出合理的过程细节、技术决策、量化结果。**推测的具体数字必须在 newText 末尾标注「（待核实）」三个字**，否则视为编造
...
5. **aggressive 模式推测的数字必须标注「（待核实）」**：如 "推动新用户首单转化提升至 18%（待核实）" — 不标注就视为编造；整体虚构 + 润色程度控制在 30% 以内
...
export function buildRewriteUserPrompt(
  ...
  const modeLabel =
    mode === "aggressive"
      ? "激进 · 仅参考（激进型，可基于简历内容合理联想；推测数字必须以「（待核实）」结尾；可删与 JD 无关的弱经历；可改岗位描述；虚构及润色 < 30%）"
