...
### moderate（适中 · 推荐）— 建议 5-12 条 changes
**用户期望**：稳妥型，虚构及润色程度 < 10%；重点突出与目标岗位匹配的【能力表现 / 业绩表现 / 项目经历】。

- **保留经历框架与顺序**：不改公司、不改历史岗位、不改时间，不重组数组顺序
- **只调措辞 / 补量化 / 对齐 JD 关键词**：换更精准的动词、显式带出 JD 要求的技术栈或方法论名词
- **基于原文已有的数字补量化优先**：原文有 "降到 1.4s" 这种数据可以保留并强化措辞；少量必要的合理推测数字必须以「（待核实）」结尾
- **不擅自删除经历**：可以 replace 让某条 highlight 更对齐，不要 delete 掉整段
- 大多数 changes 是 replace（占 70%+），少量 append（补一条与 JD 强相关但简历漏写的能力点）

### aggressive（激进 · 仅参考）— 建议 8-20 条 changes
**用户期望**：激进型 —— 措辞更强势、更敢重构呈现方式，重点突出与目标岗位匹配的【能力表现 / 业绩表现 / 项目经历】。
**激进 ≠ 编造**：aggressive 与 moderate 同样不许虚构事实，二者区别只在「改写幅度 / 重构自由度」，不在「能编多少」。

- **只改"怎么说"，不改"做过什么"**：可大幅改写措辞、换更强的动词、把 JD 关键词显性化，但每条经历改写后描述的**动作与职责必须与原文是同一件事**。
    - ✗ 反例："每日与客户邮件交流、处理售后问题" 改成 "处理大量客户数据、分类归档、确保零误差" —— 把"邮件沟通"偷换成"数据处理"，篡改了事实
    - ✓ 正例："每日与客户邮件交流、处理售后问题" 改成 "高频邮件对接客户需求、闭环跟进售后问题" —— 同一件事，措辞更利落有力
- **不发明数字**：只用原文已有的数字；原文没有量化结果时，用强动词和结果导向措辞体现价值，不要凭空造百分比 / 规模。（万一确需推测，该数字须在 newText 末尾标「（待核实）」—— 但优先不推测。）
...
4. **moderate 模式优先用原文数字**：原文有的数字保留并强化措辞；少量必要的合理推测数字必须以「（待核实）」结尾，且整体虚构 + 润色程度控制在 10% 以内
5. **aggressive 模式不篡改经历事实**：可大改措辞、可重构呈现，但不得把一条经历描述的真实动作 / 职责换成另一回事；不发明原文没有的数字（万一推测须标「（待核实）」）
...
export function buildRewriteUserPrompt(
  ...
  const modeLabel =
    mode === "aggressive"
      ? "激进 · 仅参考（激进型：措辞更强势、可重写 summary/label、可重组 skills、可建议删弱经历；不篡改经历事实、不发明数字）"
      : "适中 · 推荐（稳妥型，保留经历框架与顺序；只调措辞 + 补量化 + 对齐 JD 关键词；虚构及润色 < 10%）";
  ...
  return [
    ...
    `\n本次优化程度：${mode}（请严格按照 system 中「优化程度规则」对应一套规则执行；moderate 不得编数字、不得删经历；aggressive 只改措辞与呈现、不得篡改经历事实、不得发明数字）。`,
  ].join("\n");
}
